                      ...
                      문자함수 
                        SUBSTR => 문자 자르기 
                                         SUBSTR(컬럼명,시작위치,몇글자)
                                                              ----------- 1번부터 
                        INSTR => 문자의 위치 
                                       INSTR(컬럼명 , 찾는 문자 , 시작위치 , 몇번째)
                        LENGTH => 문자의 갯수 
                                       LENGTH(컬럼명) : 비밀번호 
                        RPAD => 글자갯수가 모자라는 경우 (지정된 문자 출력)
                                      ad***
                                      RPAD(컬럼명,문자갯수,'*')
                                                        LENGTH
                                      => 아이디 찾기 / 댓글 
                        ** REPLACE : 문자 변경 => & , | , " 
                         ...
                         => http://localhost:8080/food_detail.jsp?fno=1&page=3
                                          ...
                                          DELETE : DELETE    
                                          -----------------------------                          
                                          REPLACE(컬럼명,변경대상문자,변경할 문자)
                          => 나머지 함수 : TRIM() , INITCAP , UPPER , LOWER 
                                                  => 자바의 String으로 처리 
                      숫자함수 
                          => MOD : 나머지 => %
                               MOD(숫자1,숫자2) => 숫자1%숫자2 
               ...
               SQL문장 2개 이상 통합하는 과정 : 서브쿼리
                       ...
                       스칼라 서브쿼리 : 컬럼 대신 사용 
                        ...
                        FROM table_name 
                         ...
                         ==> 이미 SQL문장이 제작이 된 경우 
                                = 다른 테이블에서 추가적으로 데이터 추출 
                                = 유지보수 
                         ==> FUNCTION을 만들에서 처리 할 경우도 있다
                       인라인뷰 : 테이블 대신 사용 (보안)
                         SELECT *
                         FROM (SELECT ~)
                         => TOP-N : 1번부터 
                     ...
                     = 전체 데이터 저장 
                          ...
                          *** 자바에서는 자동으로 COMMIT이 된다 (autocommit())
                          ...
                          => conn.setAutoCommit(false)
                               INSERT / INSERT / INSERT / INSERT / INSERT
                               conn.commit()
                     = 원하는 데이터만 저장 = NULL 허용 , DEFAULT 설정 
          ...
          DDL : 정의 언어 
                  ...
                  TABLE 
                     ...
                     2.  정형화된 데이터  => 필요한 데이터만 첨부 
                         => NOSQL (몽고디비 , ElasticSearch)  
                         => 제약조건 
                              = 이상현상 방지 (수정 , 삭제) 
                                 = 중복이 없는 데이터 저장 = 기본키 (구분자) 
                                      : 무결성 => PRIMARY KEY
                                 = 외부 데이터 참조 = 외래키, 참조키 
                                      : 참조무결성 => FOREIGN KEY  
                              = NOT NULL : 값이 반드시 존재 
                              = UNIQUE : 중복이 없는 값 (NULL허용) 
                                   => PRIMARY KEY 대체값 
                              = CHECK : 지정된 값만 저장 
                              = PRIMARY KEY : NOT NULL + UNIQUE 
                                  => 테이블에 반드시 한개 이상을 설정 = 기본키 
                              = FOREIGN KEY : 다른 테이블의 데이터 참조 
                     테이블 
                        1) 데이터 추출 
                        2) 데이터형 설정 
                        3) 제약조건 
                     ...
                     형식)
                              테이블명 
                                 1) 한글 / 알파벳 => 대소문자 구분이 없다 
                                     실제로 오라클에 저장 (대문자로 저장) 
                                     user_tables : PRIMARY KEY (중복이 되면 안된다)
                                 2) 글자수는 30글자(byte) (10글자) 
