...
/*
	...
	서브쿼리 : 쿼리안에 다른 쿼리문장 추가 
	  ...
	  => 단점 : SQL문장이 비대 => 분석하기 어려울 수 있다
	  ---------------------------------------------------------------
	    1) 사용법 => 서브쿼리는 SELECT만 가능
		  조인 => SELECT
		  서브쿼리 => SELECT , INSERT , UPDATE , DELETE
		SELECT 뒤에 
		  SELECT ename , (SELECT ~) => 컬럼대신사용 : 스칼라서브쿼리
		FROM 뒤에 (SELECT~) => 테이블 대신 사용 : 인라인뷰 
				------------- 보안 
		WHERE 뒤에  조건값 => 서브쿼리 
				  ------------------------
				  1. 단일행 서브쿼리 => 컬럼 1개
