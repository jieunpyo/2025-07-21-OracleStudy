...
/*
	 ...
	 ** 오라클에서 한글 3byte  성별 => CHAR(6)
	 ...
	 예) data CHAR(10)   => data='A' => 메모리는 10byte이다
...
-- 산술 연산자 : SELECT => 통계 
-- 비교연산자 = : 상세보기 
-- 사원 중에 SCOTT사원의 모든 출력
SELECT *
FROM emp
WHERE ename='SCOTT';
-- 사원중에 직위(job)가 CLERK인 사원의 모든 출력
SELECT * FROM EMP WHERE JOB ='CLERK';
-- 사원의 사번이 7900인 사원의 사번(empno),이름(ename),
--입사일(hiredate),급여(sal)
SELECT EMPNO,ENAME,HIREDATE,SAL FROM EMP WHERE EMPNO=7900;
...
-- 가장 쉬운 프로그램 : 데이터베이스연동 
-- < 작다 ==> 날짜 , 문자까지 사용이 가능 
-- 사원중에 급여(sal)가 2000보다 작은 사원의 모든 출력 
SELECT *
FROM emp
WHERE sal<2000;
...
-- 사원중에 급여(sal)가 2000보다 많이 받는 사원의 모든 출력 
SELECT *
FROM emp
WHERE sal>2000;
...
-- 사원중에 급여(sal)가 2000(포함)보다 많이 받는 사원의 모든 출력 
SELECT *
FROM emp
WHERE sal>=2000;
