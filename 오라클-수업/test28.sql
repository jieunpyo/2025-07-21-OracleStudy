      ...
      1) VIEW : 테이블 참조 (필요한 데이터를 추출 후 저장)
        ...
        2) 인덱스(INDEX) 
            ...
            2) 인덱스의 종류 
                ...
                3. 고유 인덱스 
                    => 중복허용하지 않는 경우 : UNIQUE / PRIMARY KEY 
                    => 정렬
                         INDEX_ASC(테이블명,인덱스명)
               ...
               5. 불필요한 인덱스 남용 금지 
                ...
                => 252 page 
                     인덱스 생성 시기 
                      ...
                      5) insert / update/ delete많은 경우 
                          => 인덱스변경 => rebuild => 성능 저하 
                => 자료를 쉽게 빠르게 검색할수 있게 만든 데이터 구조 
                                          루트  ==> B-Tree
                                            |
                                 ---------------------
                                 |                        |  ==> 내부 루트 
...
/*
    AAAStOAAHAAAAFcAAA 
    AAAStOAAHAAAAFcAAB
    AAAStO AAH AAAAFc AAC
    --------- ----- --------- -----
                                   ROW의 구분자 
                        블록번호 
                DataFile번호 => 이미지형식으로 저장 => dbf 
     table(Number)
*/ 
-- 정렬 
/*
    ...
    ORDER BY => 대신 
     힌트 사용 => "/*+ */"  --
            +INDEX(테이블명 인덱스명) => CERATE 사용 사용자 정의 
              -------------------------- UNIQUE / PRIMARY KEY 
            +INDEX_ASC(테이블명 인덱스명)
            +INDEX_DESC(테이블명 인덱스명)
    ----------------------------------------------------------------------
