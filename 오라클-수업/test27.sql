...
/*
    ...
    -------------------------------------------------- 데이터베이스 설계 방법론 
     VIEW 
      ...
      = 보안 뛰어나다 / SQL문장을 단순화 
      = 실제 데이터가 저장되는 것이 아니라 SELECT문장을 저장하고 있다 
      1. 단순뷰 : 테이블 한개 연결 
      2. 복합뷰 : 테이블 여러개 연결 (JOIN / SUBQuery)
      3. 인라인뷰 :  FROM 뒤에 SELECT 
      ** 주의 : DML이 가능 (INSERT / UPDATE / DELETE) 
                  -----------------------------------------------
                   => 참조하는 테이블 자체가 변경 
      => WITH CHECK OPTION / READONLY OPTION (View => 읽기전용)
*/
-- 복합뷰
/*
conn system/happy
...
/*
   ...
   3. 뷰의 종류 
      ...
      복합뷰 (테이블 2개 이상 연결) : JOIN / SubQuery => 복잡한 SQL을 사용시
      인라인뷰 
   4. 목적 
       = 보안 => 가상 테이블이라 노출이 안된다 
