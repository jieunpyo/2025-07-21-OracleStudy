...
/*
	...
	협업 / 배포 
	 ...
	 => ER 모델  / 정규화 : 제약조건 , 키의 종류 
	  ...
	  ------------------------------------------------------------------------------
	  정규화  => 역정규화 
	      목적 : 이상현상이 있는 테이블을 분해해서 이상형상을 방지하는 목적
		      ------------  수정 이상 / 삭제 이상 
		       => 최대한 줄일 수 있는 방법 모색 
	      장점 
		 = 데이터베이스 변경시 이상현상을 제거 
	 ...
	 --------------------------------------------
	   2	       심청이	오라클 , HTML
	 --------------------------------------------
	   3	       박문수	 CSS,JSP,Sprint   취소시에 => 수정이 어렵다
	 --------------------------------------------
  ...
  = 3정규화 : 한개의 기본키를 가지고 테이블을 제어 
	...
	------------------------------------------------------------------
	 ...
	 요구사항
	 = 쇼핑몰은 고객, 상품, 주문을 관리한다.
	 = 고객은 고객ID, 이름, 전화번호, 주소를 가진다.
	 = 상품은 상품ID, 상품명, 가격, 재고수량을 가진다.
	 = 고객은 여러 상품을 주문할 수 있고,
	 = 한 상품도 여러 고객에게 주문될 수 있다.
	 = 주문 시 주문일자, 수량을 기록한다.
	 ERD
	   고객(고객ID(PK), 이름, 전화번호, 주소)
	   상품(상품ID(PK), 상품명, 가격, 재고수량)
	   주문(고객ID(FK), 상품ID(FK), 가격, 재고수량)
	 관계유형
	      고객 ---- 주문 ---- 상품
		     1:N	  N:1
	 요구사항
